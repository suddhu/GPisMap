...
void GPShape::reEvalPoints(std::vector<std::shared_ptr<Node> >& nodes){
    ...
    for (auto it=nodes.begin(); it != nodes.end(); it++){
        ...
        // r: (x,y) to (angle, range)
        // r0: angle to range via GP 
        ...
        if (noise > 1.0 && grad_noise > 0.61){
            ...
            // update the point to be on the surface!?
...
void GPShape::evalPoints(){
    ...
    for (int k=0; k<obs_numdata; k++){
        ...
        /////////////////////////////////////////////////////////////////
        // update the point
        p->updateData(-setting.fbias, noise, grad,grad_noise, NODE_TYPE::HIT); // update to have 0 value = surface
...
void GPShape::test_kernel(int thread_idx,
    ...
    for (int i = start_idx; i < end_idx; ++i){
        ...
        if (quads.size() == 1){
            ...
            if (gp != nullptr){
                gp->test2Dpoint(xt,res[k6],res[k6+1],res[k6+2],res[k6+3],res[k6+4],res[k6+5]); // val, gradx, grady, varval, vargradx, vargrady
            }
        }
