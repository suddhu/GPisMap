...
static float Rtimes = (float)GPISMAP2_RTIMES;
static float C_leng = (float)GPISMAP2_TREE_CLUSTER_HALF_LENGTH;
tree_param OcTree::param = tree_param((float) GPISMAP2_TREE_MIN_HALF_LENGTH,
                                      (float) GPISMAP2_TREE_MAX_HALF_LENGTH,
                                      (float) GPISMAP2_TREE_INIT_ROOT_HALF_LENGTH,
                                      C_leng);

static inline bool isRangeValid(float r)
{
    return (r < GPISMAP2_MAX_RANGE) &&  (r > GPISMAP2_MIN_RANGE);
}
...
GPisMap2::GPisMap2(GPisMap2Param par):t(0),
                                      gpo(0),
                                      obs_numdata(0),
                                      setting(par)
{
...
bool GPisMap2::preprocData(float * dataz, int N, std::vector<float> & pose)
{
    if (dataz == 0 || N < 1)
        return false;
...
void GPisMap2::update(float * dataz, int N, std::vector<float> & pose)
{
...
void GPisMap2::reEvalPoints(std::vector<std::shared_ptr<Node3> >& nodes){
...
void GPisMap2::updateGPs_kernel(int thread_idx,
                                int start_idx,
                                int end_idx,
...
void GPisMap2::updateGPs(){
    ...
    for(int i = 0; i < num_leftovers; ++i){
        threads[i] = std::thread(&GPisMap2::updateGPs_kernel,
                                 this,
                                 i,
                                 element_cursor,
...
void GPisMap2::test_kernel(int thread_idx,
                           int start_idx,
                           int end_idx,
                           float *x,
                           float *res){
...
bool GPisMap2::test(float * x, int dim, int leng, float * res){
    ...
    for(int i = 0; i < num_leftovers; ++i){
        threads[i] = std::thread(&GPisMap2::test_kernel,
                                 this,
                                 i,
                                 element_cursor,
                                 element_cursor + batch_size + 1,
...
void GPisMap2::getAllPoints(std::vector<float> & pos)
{
    pos.clear();
